...
→ Zkopíruj `.env.example` na `.env`, vyplň `OPENAI_API_KEY=` svým klíčem a ulož. Případně nastav proměnnou prostředí `OPENAI_API_KEY`, nebo zadej klíč v nastavení aplikace. Pořadí přednosti: .env / env → nastavení v aplikaci.
